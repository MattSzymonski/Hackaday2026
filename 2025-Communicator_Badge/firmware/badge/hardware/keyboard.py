...
class Keyboard:
    ...
    def __init__(self):
        ...
        # Arrow key held-state (True while the key is physically held down)
        ...
        for event in new_events:
            ...
            # Check function keys
                ...
                if bool(event[0]):
                    ...
                    self._f5 = FN_UNPRESSED
            # Arrow-key held state (tracked for both press and release)
            elif self.KEY_MATRIX[event[1]] == self.LEFT:
                self.left_held = bool(event[0])
                if event[0]:
                    self.keybuffer.append(self.LEFT)
            elif self.KEY_MATRIX[event[1]] == self.RIGHT:
                self.right_held = bool(event[0])
                if event[0]:
                    self.keybuffer.append(self.RIGHT)
            elif self.KEY_MATRIX[event[1]] == self.UP:
                self.up_held = bool(event[0])
                if event[0]:
                    self.keybuffer.append(self.UP)
            elif self.KEY_MATRIX[event[1]] == self.DOWN:
                self.down_held = bool(event[0])
                if event[0]:
                    self.keybuffer.append(self.DOWN)
